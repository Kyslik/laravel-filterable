...
class GenericFilterTest extends TestCase
{
    ...
    {
        $filter = $this->buildFilter($this->prefix . 'username=joe');
    ...
    {
        $filter = $this->buildFilter($this->prefix . 'username=><joe,jane');
        $this->assertEquals('select * where "username" between \'jane\' and \'joe\'',
            $this->dumpQuery($filter->apply($this->builder)));

        $this->resetBuilder();

        $filter = $this->buildFilter($this->prefix . 'username=><jane,joe');
        $this->assertEquals('select * where "username" between \'jane\' and \'joe\'',
            $this->dumpQuery($filter->apply($this->builder)));
    }
    ...
    {
        $filter = $this->buildFilter($this->prefix . 'username=i=jane,joe');
    ...
    {
        $filter = $this->buildFilter($this->prefix . 'username=b=1');
        $this->assertEquals('select * where "username" = \'1\'', $this->dumpQuery($filter->apply($this->builder)));
        $this->resetBuilder();
        $filter = $this->buildFilter($this->prefix . 'username=b=true');
        $this->assertEquals('select * where "username" = \'1\'', $this->dumpQuery($filter->apply($this->builder)));
        $this->resetBuilder();
        $filter = $this->buildFilter($this->prefix . 'username=b=yes');
        $this->assertEquals('select * where "username" = \'1\'', $this->dumpQuery($filter->apply($this->builder)));
    }
    ...
    {
        $filter = $this->buildFilter($this->prefix . 'username=b=0');
        $this->assertEquals('select * where "username" = \'0\'', $this->dumpQuery($filter->apply($this->builder)));
        $this->resetBuilder();
        $filter = $this->buildFilter($this->prefix . 'username=b=false');
        $this->assertEquals('select * where "username" = \'0\'', $this->dumpQuery($filter->apply($this->builder)));
        $this->resetBuilder();
        $filter = $this->buildFilter($this->prefix . 'username=b=no');
        $this->assertEquals('select * where "username" = \'0\'', $this->dumpQuery($filter->apply($this->builder)));
    }
    ...
    {
        $now    = Carbon::now();
        $filter = $this->buildFilter($this->prefix . 'created_at=t='.$now->timestamp);
    ...
    function test_generic_filter_timestamp_between()
    {
        ...
        $filter = $this->buildFilter($this->prefix . 'username=t><'.$now->timestamp.','.$then->timestamp);
        $this->assertEquals('select * where "username" between \''.$now->toDateTimeString().'\' and \''.$then->toDateTimeString().'\'',
            $this->dumpQuery($filter->apply($this->builder)));

        $this->resetBuilder();

        $filter = $this->buildFilter($this->prefix . 'username=t><'.$then->timestamp.','.$now->timestamp);
        $this->assertEquals('select * where "username" between \''.$now->toDateTimeString().'\' and \''.$then->toDateTimeString().'\'',
            $this->dumpQuery($filter->apply($this->builder)));
    }
